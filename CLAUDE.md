...
- `src/components/` — UI: `Nav`, `Hero` + `HeroXp` (client age/XP island), `Stats`, `Tools`, `Platforms`, `Skills` + `SkillRow`, `Resume`, `Lore`, `Contact`, `ContactForm` / `ConsultingForm` (Web3Forms via `lib/web3forms.ts`), `Subscribe` / `SubscribeBlock` (native D1 list), `Gallery`, `FloatingActions`, `BootSequence`, `KonamiEasterEgg`, `Counter`.
